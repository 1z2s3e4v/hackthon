...
void SA::cal_weight() 
{
    _center_pallet_x = 0. + _container[0] / 2.;
    _center_pallet_y = 0. + _container[1] / 2.;
    _weight_ul = 0; // total weight of upper-left of pallet
    _weight_bl = 0; // total weight of below-left of pallet
    _weight_ur = 0; // total weight of upper-right of pallet
    _weight_br = 0; // total weight of below-right of pallet
    ...
    for(size_t i = 0; i < _n; i++){
        ...
        size_t d = _boxV[i]->_deepth;
        size_t w = _boxV[i]->_width;
        size_t h = _boxV[i]->_height;
        size_t weight = _boxV[i]->_weight;

        // Box is all in upper-left
        if (x <= _center_pallet_x && y >= _center_pallet_y && x+d <= _center_pallet_x && y+w >= _center_pallet_y) { 
            _weight_ul += weight;
        }

        // Box is all in below-left
        else if (x <= _center_pallet_x && y <= _center_pallet_y && x+d <= _center_pallet_x && y+w <= _center_pallet_y) { 
            _weight_bl += weight;
        }

        // Box is all in below-right
        else if (x >= _center_pallet_x && y <= _center_pallet_y && x+d >= _center_pallet_x && y+w <= _center_pallet_y) { 
            _weight_br += weight;
        }

        // Box is all in upper-right
        else if (x >= _center_pallet_x && y >= _center_pallet_y && x+d >= _center_pallet_x && y+w >= _center_pallet_y) { 
            _weight_ur += weight;
        }

        // Box has some part in upper-left and has some part in below-left
        else if (x <= _center_pallet_x && y <= _center_pallet_y && x+d <= _center_pallet_x && y+w >= _center_pallet_y) { 
            double total_area = d*w;
            double ul_area = d*(y+w-_center_pallet_y);
            double br_area = total_area - ul_area;
            _weight_ul += weight * (ul_area / total_area);
            _weight_bl += weight * (br_area / total_area);
        }

        // Box has some part in upper-left and has some part in upper-right
        else if (x <= _center_pallet_x && y >= _center_pallet_y && x+d >= _center_pallet_x && y+w >= _center_pallet_y) { 
            double total_area = d*w;
            double ul_area = w*(x+d-_center_pallet_x);
            double ur_area = total_area - ul_area;
            _weight_ul += weight * (ul_area / total_area);
            _weight_ur += weight * (ur_area / total_area);
        }

        // Box has some part in upper-right and has some part in below-right
        else if (x >= _center_pallet_x && y <= _center_pallet_y && x+d >= _center_pallet_x && y+w >= _center_pallet_y) { 
            double total_area = d*w;
            double ur_area = d*(y+w-_center_pallet_y);
            double br_area = total_area - ur_area;
            _weight_ur += weight * (ur_area / total_area);
            _weight_br += weight * (br_area / total_area);
        }

        // Box has some part in below-left and has some part in below-right
        else if (x <= _center_pallet_x && y <= _center_pallet_y && x+d >= _center_pallet_x && y+w <= _center_pallet_y) { 
            double total_area = d*w;
            double bl_area = w*(x+d-_center_pallet_x);
            double br_area = total_area - bl_area;
            _weight_bl += weight * (bl_area / total_area);
            _weight_br += weight * (br_area / total_area);
        }

        // Box are separated by four parts
        else {
            double total_area = d*w;
            double ul_area = (_center_pallet_x-x) * (y+w-_center_pallet_y);
            double bl_area = (_center_pallet_x-x) * (_center_pallet_y-y);
            double ur_area = (x+d-_center_pallet_x) * (y+w-_center_pallet_y);
            double br_area = total_area - (ul_area + bl_area + ur_area);
            _weight_ul += weight * (ul_area / total_area);
            _weight_bl += weight * (bl_area / total_area);
            _weight_ur += weight * (ur_area / total_area);
            _weight_br += weight * (br_area / total_area);
    ...
    _total_weight = _weight_ul + _weight_bl + _weight_ur + _weight_br;

    _part_x = (_weight_ul + _weight_bl - _weight_ur - _weight_br) / _total_weight;
    _part_y = (_weight_ul + _weight_ur - _weight_bl - _weight_br) / _total_weight;
...
pair<double, double> SA::calCost(vector<string> gammaV, vector<size_t> orientV)
{
    ...
    if(_part_x >= 0 && abs(_part_x) > 0.15){
        // cout << "left is too heavy" << endl;
        scaledCost *= pow(abs(_part_x)/0.15 , 5);
        isValid = false;
    }
    if(_part_y >= 0 && abs(_part_y) > 0.15){
        // cout << "Up is too heavy" << endl;
        scaledCost *= pow(abs(_part_y)/0.15, 5);
        isValid = false;
    }
...
void SA::updateBoxInfo(vector<string> gammaV, vector<size_t> orientV)
{
    ...
    // =========================  Whether do Final Shiffting ====================== //

    // If the left or right part is heavier than the opposite one, then do shiffting

    size_t x_left = 0, x_right = globalMaxRear, y_left = 0, y_right = globalMaxRight;
    if ( _part_x > 0 && abs(_part_x) > 0.1)
    {   
        cout << "Too heavy in the left part! " << endl;
        double diff = _container[0]-globalMaxRear - 1;
        for (size_t i=0; i< abs(diff); ++i){
            if (x_right == _container[0]) break;
            for (size_t j=0; j<_n; ++j){
                int x_tmp = _boxV[j]->_x ;
                // cout << "Original: " << x_tmp << ", " <<_boxV[j]->_y;
                // _boxV[j]->_x = _boxV[j]->_x + (i+1);
                _boxV[j]->_x += 1;
                x_right = max(x_right, _boxV[j]->_x);
                // cout << ", After: " << _boxV[j]->_x << ", "<< _boxV[j]->_y << endl;
            }
            cal_weight();
            if( abs(_part_x) <= 0.1 )
            {   
    ...
    else if ( _part_x < 0 && abs(_part_x) > 0.1)
    {   
        cout << "Too heavy in the right part! " << endl;
        double diff = _container[0]-globalMaxRear - 1;
        for (size_t i=0; i< abs(diff); ++i){
            if (x_left == 0) break;
            for (size_t j=0; j<_n; ++j){
    ...
    // If the left or right part is heavier than the opposite one, then do shiffting
    if ( _part_y > 0 && abs(_part_y) > 0.1)
    {   
        cout << "Too heavy in the upper part! " << endl;
        double diff = _container[1]-globalMaxRight - 1;
        for (size_t i=0; i< abs(diff); ++i){
            if (y_left == 0) break;
            for (size_t j=0; j<_n; ++j){
                int y_tmp = _boxV[j]->_y ;
                // cout << "Original: " << _boxV[j]->_x << ", " << y_tmp;
                // _boxV[j]->_y = _boxV[j]->_y - (i+1);
                _boxV[j]->_y -= 1;
                y_left = min(y_left, _boxV[j]->_y);
                // cout << ", After: " << _boxV[j]->_x << ", "<< _boxV[j]->_y << endl;
            }
            cal_weight();
            if( abs(_part_y) <= 0.1 )
    ...
    else if ( _part_y < 0 && abs(_part_y) > 0.1)
    {
        cout << "Too heavy in the below part! " << endl;
        double diff = _container[1]-globalMaxRight - 1;
        for (size_t i=0; i< abs(diff); ++i){
            if (y_right == _container[1]) break;
            for (size_t j=0; j<_n; ++j){
                int y_tmp = _boxV[j]->_y ;
                // cout << "Original: " << _boxV[j]->_x << ", " << y_tmp;
                // _boxV[j]->_y = _boxV[j]->_y + (i+1);
                _boxV[j]->_y += 1;
                y_right = max(y_right, _boxV[j]->_y);
                // cout << ", After: " << _boxV[j]->_x << ", "<< _boxV[j]->_y << endl;
            }
            cal_weight();
            if( abs(_part_y) <= 0.1 )
